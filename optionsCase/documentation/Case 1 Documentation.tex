...
The implementation of this method should be ready to provide current market quotes at any point in time. Broker request submitted at each time interval will cause this method to be called. The name of the \verb+Quote+ variable within \verb+QuoteList+ indicates the strike. Therefore, please provide your price with the correct corresponding \verb+Quote+ object. 
...
If the QuoteList object returned in \verb getCurrentQuotes  matches against any broker orders, the \verb newFill  method will be invoked with trade details. The \verb strike  argument indicates which option was filled. Definition of \verb direction  will stay consistent as before in reference to the broker, i.e., -1 would be mean that you have {\it bought} one lot with a broker {\it sell} order. 

If the QuoteList object returned in \verb getCurrentQuotes  does not result in a trade, the \verb noBrokerFills  method will be invoked so that your implementation can make any necessary adjustments.
...
The \verb amount  argument reflects the monetary amount of the penalty that the participants have incurred in that particular round. 

In addition, we have also provided you with the Black-Scholes option pricing function and vega calculation function in \verb+OptionsMathUtils.java+:
\begin{verbatim}
